...
const InfoSection = () => {
    const infoItems = [
        {
            icon: <FaShippingFast className="text-5xl text-red-600" />,
            title: 'Free Shipping',
        ...
        <div className="bg-white pb-8 pt-12">
            <div className="container mx-auto grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4">
                {infoItems.map((item, index) => (
                    <div
                        key={index}
                        className="flex flex-col items-center text-center p-6 border rounded-lg shadow-md transform transition-all duration-300 hover:scale-105 hover:border-red-600 hover:shadow-lg cursor-pointer"
                    >
                        {item.icon}
